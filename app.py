...
# --- KONFIGURACJA ---
# Ustawiamy od razu ciemny motyw dla całego layoutu Streamlit (jeśli wymuszone w ustawieniach, to będzie spójne)
...
def calculate_accurate_portfolio(df_closed: pd.DataFrame, df_cash: pd.DataFrame) -> tuple:
    # 1. Zrealizowany zysk (Zysk z zamkniętych + Dywidendy - Podatki + Odsetki)
    ...
    # 2. Wyliczanie posiadanych akcji na bazie Cash Operations
...
def generate_weekly_historical_timeline(df_cash: pd.DataFrame) -> pd.DataFrame:
    ...
    for current_day in weekly_range:
        ...
        # Wkład netto na dany dzień (Depozyty + Transfery ujemne PLN to USD)
        ...
        # Właściwa wolna gotówka na koncie w tamtym dniu
                ...
                for t_symbol, shares in shares_per_ticker.items():
                    if shares > 0.0001:
                        ...
                        if pd.isna(price) or price is None:
                            # Szacunek jeśli brak notowań giełdowych
                            price = 0.0 
        ...
        # Realna wartość to suma posiadanej wtedy gotówki i wyceny akcji
...
try:
    ...
    with st.spinner("Przetwarzanie danych i trybu Dark Mode..."):
    ...
    if response.status_code == 200:
        ...
        # 2. Wolna Gotówka (Suma wszystkich operacji Cash Operations to aktualne saldo XTB)
        current_free_cash = df_cash["Amount"].sum()
        
        # 3. Wartość Portfela (Akcje + Gotówka)
        total_portfolio_value = total_value_stocks + current_free_cash
        
        # 4. Wkład Netto
        deposits_only = df_cash[df_cash["Type"] == "Deposit"]["Amount"].sum()
        to_usd_transfers = df_cash[(df_cash["Type"] == "Transfer") & (df_cash["Comment"].str.contains("PLN to USD", na=False, case=False))]["Amount"].sum()
        total_invested = deposits_only + to_usd_transfers
        
        # 5. Wynik Portfela (Wartość - Wkład)
        ...
        # Górny rząd: Składniki portfela
        col1, col2, col3 = st.columns(3)
        col1.metric("Wycena Akcji", f"{total_value_stocks:,.2f} zł")
        col2.metric("Wolne Środki na koncie", f"{current_free_cash:,.2f} zł")
        col3.metric("Wartość Portfela", f"{total_portfolio_value:,.2f} zł")
        ...
        col5.metric("Niezrealizowany Wynik (Zysk/Strata)", f"{total_gain_pln:,.2f} zł", delta=f"{roi:.2f}%")
        col6.metric("Zrealizowany Zysk 🟢 (Historia)", f"{realized_pnl:,.2f} zł")

        st.markdown("---")

        # --- WYKRES LINIOWY (DARK MODE + HOVER) ---
        st.subheader("📈 Zmiana Wartości Portfela (z uwzględnieniem Wolnych Środków)")
        
        # Kolor linii w zależności od tego, czy portfel jest na plusie czy minusie
        line_color = '#00E676' if total_gain_pln >= 0 else '#FF1744' # Neon green lub Neon red
        ...
        fig_line.add_trace(go.Scatter(
            ...
            mode='lines', name='Twój Wkład Netto',
            line=dict(color='#B0BEC5', width=2, dash='dash'), # Jasnoszary
            hovertemplate="Data: %{x}<br>Wkład Netto: %{y:,.2f} zł<extra></extra>"
        ))
        
        # Linia wartości portfela (z customdata dla wyniku)
        fig_line.add_trace(go.Scatter(
            x=timeline_df["Time"], y=timeline_df["Wartość"],
            mode='lines', name='Wartość Portfela',
            customdata=timeline_df["Wynik"], # Przekazujemy wynik do dymka!
            line=dict(color=line_color, width=3),
            hovertemplate="Data: %{x}<br>Wartość Portfela: %{y:,.2f} zł<br><b>Wynik: %{customdata:,.2f} zł</b><extra></extra>"
        ))
        
        fig_line.update_layout(
            template="plotly_dark", # DARK MODE WŁĄCZONY
            hovermode="x unified",
            xaxis_title="",
        ...
        with chart_col2:
            ...
            if not final_portfolio.empty:
                fig_bar = px.bar(
                    ...
                    color_continuous_scale="greens" if total_gain_pln >= 0 else "reds"
                )
